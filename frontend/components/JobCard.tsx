...
const MatchBar: React.FC<{ score: number }> = ({ score }) => {
  const color =
    score >= 80 ? 'from-emerald-500 to-green-400 shadow-emerald-500/20' :
    score >= 60 ? 'from-amber-500 to-yellow-400 shadow-amber-500/20' :
  ...
  return (
    <div className="flex items-center gap-3 mt-1.5">
      <div className="flex-1 h-2 bg-slate-800/50 rounded-full overflow-hidden border border-white/5">
        <div
          className={cn("h-full rounded-full bg-gradient-to-r transition-all duration-1000 shadow-[0_0_10px_rgba(0,0,0,0.1)]", color)}
          style={{ width: `${Math.min(score, 100)}%` }}
        />
      </div>
      <span className={cn("text-[10px] font-black tabular-nums tracking-tighter", score >= 80 ? "text-emerald-400" : score >= 60 ? "text-amber-400" : "text-rose-400")}>
        {score}%
      </span>
    </div>
  );
...
const JobCard: React.FC<JobCardProps> = ({
  ...
  const skills = Array.isArray(targetJob.required_skills) 
    ...
    : typeof targetJob.required_skills === 'string' 
      ? targetJob.required_skills.split(',').map(s => s.trim())
      : [];
    ...
    >
      ...
      <div className="flex justify-between items-start gap-4 mb-5">
        <div className="flex-1 min-w-0">
          ...
          <h3 className="text-lg font-black text-text group-hover:text-primary-300 transition-colors line-clamp-1 tracking-tight leading-tight">
            {title}
          </h3>
          <div className="flex items-center gap-2 mt-1">
            <div className="w-5 h-5 rounded-md bg-background border border-border/50 flex items-center justify-center overflow-hidden shrink-0">
        ...
        {matchScore > 0 && <ScoreBadge score={matchScore} size="sm" />}
      </div>
      ...
      {/* Rationale / AI Insight */}
      {matchReason && (
        <div className="mb-6 p-3 bg-primary-500/5 rounded-2xl border border-primary-500/10 relative overflow-hidden group/insight">
      ...
      {/* Application Actions */}
      <div className="mt-auto pt-4 border-t border-border/30 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSave}
            className={cn(
                "w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-300 transform-gpu active:scale-90",
                saved 
                    ? "bg-primary-500/20 text-primary-400 border border-primary-500/30 shadow-glow" 
                    : "bg-surface border border-border/50 text-text-muted hover:text-text hover:border-border group/bookmark"
            )}
            title={saved ? 'Synced' : 'Bookmark'}
          >
            {saved 
                ? <BookmarkCheck size={18} /> 
                : <Bookmark size={18} className="group-hover/bookmark:scale-110 transition-transform" />
            }
          </button>
        </div>
