...
const Input: React.FC<InputProps> = ({ label, error, className = '', icon: Icon, ...props }) => {
  return (
    <div className={cn("flex flex-col gap-2", className)}>
      ...
      {error && (
        <p className="text-[10px] font-bold text-rose-500 ml-1 mt-0.5 animate-in fade-in slide-in-from-top-1 duration-200">
          {error}
        </p>
      )}
    </div>
