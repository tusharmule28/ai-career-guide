    ...
    <div className="section-container animate-fade-in">
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-8 md:mb-12 gap-6 text-center lg:text-left">
        <div className="max-w-2xl mx-auto lg:mx-0">
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-primary-500/10 rounded-full text-[10px] font-black uppercase tracking-widest text-primary-400 mb-6 border border-primary-500/20 shadow-sm">
             <Sparkles size={12} /> Curated for You
          </div>
          <h1 className="text-3xl md:text-5xl font-black text-text tracking-tight leading-tight mb-4">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
          <div className="relative w-full sm:min-w-[400px] group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-primary-400 transition-colors" size={20} />
            <input 
              type="text"
              placeholder="Search roles, companies or skills..."
              className="w-full pl-12 pr-4 py-4 bg-surface border border-border/50 rounded-2xl text-sm font-medium focus:ring-4 focus:ring-primary-400/10 focus:border-primary-400 outline-none transition-all shadow-soft group-hover:border-border"
              value={searchTerm}
      ...
      {/* Filter Bar */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6 mb-12 p-2 bg-surface/30 rounded-2xl border border-border/50">
        <div className="flex flex-wrap items-center gap-2">
          <div className="px-4 py-2 flex items-center gap-2 text-[11px] font-black text-text-secondary uppercase tracking-widest bg-surface rounded-xl shadow-soft border border-border/50">
            <Filter size={14} className="text-primary-400" />
            Filters
          </div>
          
          <select 
            className="bg-surface border border-border/50 text-xs font-bold rounded-xl px-4 py-2 outline-none focus:ring-4 focus:ring-primary-400/10 text-text-secondary cursor-pointer transition-all hover:border-primary-400 shadow-soft"
        ...
        <Button 
          variant="ghost" 
          size="sm" 
          className="text-xs font-black text-text-muted hover:text-primary-400"
          onClick={() => {
            setSearchTerm('');
            ...
            {filteredMatches.length > 0 ? (
              <section className="animate-slide-up">
                <div className="flex items-center gap-4 mb-10">
                  <div className="w-12 h-12 bg-primary-500/10 text-primary-400 rounded-2xl flex items-center justify-center shadow-soft">
                    <Sparkles size={24} />
                  </div>
                  <div>
                    ...
                    <p className="text-sm font-medium text-text-secondary">Curated based on your expertise and preferences.</p>
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            {!onlyMatched && (
              <section className="animate-slide-up" style={{ animationDelay: '0.1s' }}>
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-surface text-text-secondary rounded-lg">
                    <Filter size={20} />
                  </div>
                  <div>
                    <h2 className="text-xl font-bold text-text">All Latest Roles</h2>
                    <p className="text-sm text-text-secondary">Explore global remote opportunities.</p>
                  </div>
                </div>
