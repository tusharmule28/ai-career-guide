...
const JobCard: React.FC<JobCardProps> = ({
  ...
  return (
    <Card className={`group relative flex flex-col h-full border transition-all duration-300 overflow-hidden bg-slate-900/80 hover:bg-slate-900 border-slate-800 hover:border-slate-700 ${
      highlight ? 'ring-1 ring-violet-500/30' : ''
    }`}>
      {highlight && (
        <div className="absolute top-3 right-3 text-violet-400 animate-pulse">
          <Sparkles size={14} />
        </div>
      )}
        ...
        <div className="flex-1 pr-4">
          <div className="flex items-center gap-2 mb-1.5">
            <span className={`px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-md ${
              highlight ? 'bg-violet-600/20 text-violet-300 border border-violet-700/40' : 'bg-slate-800 text-slate-400 border border-slate-700'
            }`}>
          ...
          <h3 className="text-base font-bold text-slate-100 group-hover:text-white transition-colors line-clamp-1 leading-snug">
            {title}
          </h3>
          ...
          <div className="flex items-center gap-1.5 text-xs text-slate-400 mt-0.5">
            {targetJob.company_logo ? (
              ...
              <Briefcase size={11} className="text-slate-600 shrink-0" />
            )}
      ...
      {/* AI match reason */}
      {matchReason && (
        <div className="flex items-start gap-1.5 mb-3 px-2.5 py-2 bg-violet-950/40 border border-violet-800/30 rounded-lg">
          <Sparkles size={11} className="text-violet-400 mt-0.5 shrink-0" />
          <p className="text-[10px] text-violet-300/80 leading-tight">{matchReason}</p>
        </div>
      )}

      {/* Meta pills */}
      <div className="flex flex-wrap gap-1.5 mb-4">
        <div className="inline-flex items-center gap-1 text-[10px] font-medium text-slate-400 bg-slate-800/60 px-2 py-1 rounded-lg border border-slate-700/50">
      ...
      <div className="mt-auto pt-3 border-t border-slate-800 flex items-center justify-between gap-2 flex-wrap">
        <div className="flex gap-2 items-center">
          ...
          <button
            ...
            className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
              saved ? 'text-violet-400 bg-violet-900/30 border border-violet-700/40' : 'text-slate-500 hover:text-violet-400 hover:bg-slate-800 border border-slate-700/50'
            }`}
        ...
        <Button
          ...
          className="h-8 px-3 text-xs font-semibold text-slate-300 hover:text-white border-slate-700 hover:border-slate-600 hover:bg-slate-800"
        >
          Details
